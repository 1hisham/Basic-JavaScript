...
submitBtn.addEventListener("click", () => {
  ...
  let totalLines = (Number(locHtml.value) + Number(locJs.value) + Number(locCss.value))
  let objLoc = {
...
function calculateValues(obj) {
  ...
  let codeBalanceMin = (Number(items.codeMinutes) + Number(obj.codeMinutes)) / 60;
  let codeAddHour = 0;
  if (codeBalanceMin >= 1) {
    codeAddHour = 1;
  }
  ...
  // active code
  let activeBalanceMin = (Number(items.activeMinutes) + Number(obj.activeMinutes)) / 60;
  let activeAddHour = 0;
  ...
  //focus
  let focusBalanceMin = (Number(items.focusMinutes) + Number(obj.focusMinutes)) / 60;
  let focusAddHour = 0;
  ...
  // project
  let projectBalanceMin = (Number(items.projectMinutes) + Number(obj.projectMinutes)) / 60;
  ...
  let newObj = {
    ...
    activeHours:  Number(items.activeHours) + Number(obj.activeHours) + activeAddHour,
    activeMinutes: activeMinutes,
    focusHours: Number(items.focusHours) + Number(obj.focusHours) + focusAddHour,
    focusMinutes: focusMinutes,
    projectHours: Number(items.projectHours) + Number(obj.projectHours) + projectAddHour,
